...
const float cons8 = float(1.000000);
in vec3 varnormal;
const vec4 cons11 = vec4(1.000000, 1.000000, 0.986645, 1.000000);
const vec4 cons18 = vec4(0.000000, 0.000000, 0.000000, 1.000000);
const float cons19 = float(0.000000);
...
void main()
{
	...
	node_tex_image(var0, samp0, tmp3, tmp4);
	srgb_to_linearrgb(tmp3, tmp6);
	node_emission(tmp6, cons8, facingnormal, tmp10);
	node_bsdf_transparent(cons11, tmp12);
	node_mix_shader(tmp4, tmp12, tmp10, tmp16);
	node_output_material(tmp16, cons18, cons19, tmp20);
	linearrgb_to_srgb(tmp20, tmp22);
